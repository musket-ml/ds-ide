 ...
 * @since 3.0
 */
public class TitledHyperlink extends Hyperlink {
	/**
	 ...
	 */
	public TitledHyperlink(Composite parent, int style) {
...
//		Rectangle ibounds = image != null ? image.getBounds() : new Rectangle(0, 0, 0, 0);
		Point maxImageSize = computeMaxImageSize();
		int spacing = image!=null?textSpacing:0;
		int textWidth = bounds.width - maxImageSize.x - spacing
				- marginWidth - marginWidth;
//		int y = bounds.y+marginHeight + maxsize.y / 2 - ibounds.height / 2;
		
		Point textSize = computeTextSize(SWT.DEFAULT, SWT.DEFAULT);
		
		int imgX = Math.max(0, (bounds.width - maxImageSize.x ) / 2 - marginWidth);
		int y = marginHeight;
		int textX = Math.max(0, (bounds.width - textSize.x ) / 2 - marginWidth);
		
		if (image != null) {
			gc.drawImage(image, imgX, y);
			y += maxImageSize.y;
		}
		y += spacing;
		if (getText() != null) {
			Color oldColor = getForeground();
			if (state == HOVER && hoverFgColor != null) {
//				setRedraw(false); //TODO causes subsequent repaint. Need to avoid this
//				setForeground(hoverFgColor);
			}
			drawText(gc, bounds, textX, y, textWidth);
...
//		}
	...
	private int drawText(GC gc, Rectangle clientArea, int textX, int textY, int textWidth) {
		Point textSize = computeTextSize(SWT.DEFAULT, SWT.DEFAULT);
	...
	public void setHoverForeground(Color hoverFgColor) {
